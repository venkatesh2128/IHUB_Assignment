...
const errorHandler = (error, workerMeta) => {
  logging.info(
    `Capturing error for worker during execution of job: ${workerMeta.name}`
  );
  logging.error(error);
...
const initTestMode = () => {
  ...
  setInterval(() => {
    logging.warn(
      `${jobManager.queue.length()} jobs in the queue. Idle: ${jobManager.queue.idle()}`
    );
    ...
    if (Object.keys(jobManager.bree.workers).length) {
      logging.warn(
        `${
          Object.keys(jobManager.bree.workers).length
        } jobs running: ${runningScheduledjobs}`
      );
    }
    ...
    if (Object.keys(jobManager.bree.intervals).length) {
      logging.warn(
        `${
          Object.keys(jobManager.bree.intervals).length
