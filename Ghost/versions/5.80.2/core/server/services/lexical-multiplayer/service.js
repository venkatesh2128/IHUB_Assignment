...
module.exports = {
  async init(ghostServer) {
    _enable = async () => {
      if (_isClosing) {
        logging.info(
          "Waiting for previous Lexical multiplayer websockets service to close"
        );
        await _closePromise;
      }
      ...
      if (labs.isSet("lexicalMultiplayer")) {
        ...
        wss.on("connection", (socket, request) => {
          ...
          const docName = request.url.replace(
            "/ghost/api/admin/posts/multiplayer/",
            ""
          );
  ...
  async enable() {
    if (!_enable) {
      logging.error(
        "Lexical multiplayer service must be initialized before it can be enabled/disabled"
      );
      return;
    }
    return _enable();
